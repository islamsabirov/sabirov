"""
KINO BOT v6.0
Modern UI + PostgreSQL + Clean Cancel System
"""
import asyncio
import logging
import os
...
def cancel_kb():
    return ReplyKeyboardMarkup([[CANCEL_BTN]], resize_keyboard=True, one_time_keyboard=False)

# ═══════════════════════════════
# YORDAMCHILAR
# ═══════════════════════════════

async def safe_edit(ctx, chat_id, msg_id, text, parse_mode=None, markup=None):
    try:
        ...
        logging.error("safe_edit xato: %s", e)
        return False


async def safe_edit_caption(ctx, chat_id, msg_id, caption, parse_mode=None, markup=None):
    try:
        await ctx.bot.edit_message_caption(
            chat_id=chat_id, message_id=msg_id,
            caption=caption, parse_mode=parse_mode, reply_markup=markup
        )
        return True
    except BadRequest as e:
        if "Message is not modified" in str(e):
        ...
        logging.error("safe_edit_caption xato: %s", e)
        return False
...
async def must_subscribe(bot, uid):
    ...
    non_tg = [c for c in all_chs if c["type"] in ("private", "link")]
    shown = unsubbed + [c for c in non_tg if c["id"] not in [x["id"] for x in unsubbed]]
    blocked = len(unsubbed) > 0 or len(non_tg) > 0
...
async def cancel(update: Update, ctx):
    ...
    mkb = kb.admin_kb() if db.is_admin(uid) else kb.user_kb()
    await update.message.reply_text(
        "❌ Amal bekor qilindi",
        reply_markup=ReplyKeyboardRemove()
    )
    await update.message.reply_text("Asosiy menyu:", reply_markup=mkb)
    return END
...
async def cb_cancel(update: Update, ctx):
    ...
    await q.answer()
    ctx.user_data.clear()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id, "❌ Bekor qilindi.")
...
async def cb_chk_sub(update: Update, ctx):
    ...
    blocked, show_channels = await must_subscribe(ctx.bot, u.id)
    if blocked:
        await q.answer("⚠️ Hali barcha kanallarga obuna bo'lmadingiz!", show_alert=True)
        return
    mkb = kb.admin_kb() if db.is_admin(u.id) else kb.user_kb()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "✅ Obuna tasdiqlandi! Kino kodini yuboring.")
    await ctx.bot.send_message(u.id, "👇 Asosiy menyu:", reply_markup=mkb)
...
async def msg_find_movie(update: Update, ctx):
    ...
    if movie["is_pro"] and not db.has_sub(u.id):
        ...
        text = (
            ...
            "📅 1 Oy — " + "{:,}".format(int(price_1m)) + " so'm\n"
            "📅 3 Oy — " + "{:,}".format(int(price_3m)) + " so'm\n"
            "📅 1 Yil — " + "{:,}".format(int(price_1y)) + " so'm\n\n"
            "Obuna sotib olish uchun <b>👤 Profilim</b> tugmasini bosing."
        )
        await update.message.reply_text(
            text, parse_mode=H,
    ...
    ch = get_movie_ch()
    if not ch:
        await update.message.reply_text("⚠️ Kino kanal sozlanmagan. Admin bilan bog'laning.")
        return

    try:
        await ctx.bot.copy_message(chat_id=u.id, from_chat_id=ch,
                                    message_id=int(movie["msg_id"]))
        db.increment_views(code)
    except TelegramError as e:
        logging.error("copy_message xato: %s", e)
        await update.message.reply_text("❌ Kinoni yuborishda xatolik. Kod: " + code)
...
async def msg_profile(update: Update, ctx):
    ...
    if sub and info:
        plan_names = {
            "1_month": "1 Oy", "3_month": "3 Oy",
            "1_year": "1 Yil", "referral_bonus": "Referral Bonus"
        }
        plan_label = plan_names.get(info["plan"], "")
        expires = info["expires_at"][:10]
        sub_text = "✅ Faol (" + expires + " gacha) — " + plan_label
    else:
        sub_text = "❌ Obuna yo'q"
...
async def cb_buy_sub(update: Update, ctx):
    ...
    text = (
        ...
        "📅 1 Oy — <b>" + "{:,}".format(int(price_1m)) + " so'm</b>\n"
        "📅 3 Oy — <b>" + "{:,}".format(int(price_3m)) + " so'm</b>\n"
        "📅 1 Yil — <b>" + "{:,}".format(int(price_1y)) + " so'm</b>"
    )
...
async def cb_plan(update: Update, ctx):
    ...
    text = (
        "📅 <b>" + plan_info["label"] + " — " + "{:,}".format(int(price)) + " so'm</b>\n\n"
        "💳 To'lov usulini tanlang:"
    )
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    text, parse_mode=H, markup=kb.buy_kb())
...
async def cb_back_to_plans(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    text, parse_mode=H, markup=kb.plan_kb())
...
async def cb_pay_card(update: Update, ctx):
    ...
    text = (
        "💳 <b>" + names.get(card_type, "") + " orqali to'lov</b>\n\n"
        "📅 Tarif: <b>" + plan_info["label"] + "</b>\n"
        "💰 Summa: <b>" + "{:,}".format(int(price)) + " so'm</b>\n\n"
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    text, parse_mode=H, markup=kb.back_kb("back_to_plans"))
    await ctx.bot.send_message(
        q.from_user.id,
...
async def rcv_screenshot(update: Update, ctx):
    ...
    if not update.message.photo:
        await update.message.reply_text("📸 Chek rasmini yuboring!", reply_markup=cancel_kb())
        return S_PAY
    ...
    if not pay_id:
        await update.message.reply_text(
            "⚠️ Kutilayotgan to'lov mavjud. Admin tasdiqlashini kuting.",
            reply_markup=ReplyKeyboardRemove()
        )
        await update.message.reply_text("Asosiy menyu:", reply_markup=kb.user_kb())
        return END
    await update.message.reply_text(
        "✅ To'lovingiz qabul qilindi! Tez orada tasdiqlanadi.",
        reply_markup=ReplyKeyboardRemove()
    )
    await update.message.reply_text("Asosiy menyu:", reply_markup=kb.user_kb())
    names = {"uzcard": "UzCard", "humo": "Humo", "visa": "Visa/MasterCard"}
...
async def cb_pay_ok(update: Update, ctx):
    ...
    if not pay or pay["status"] != "pending":
        await q.answer("⚠️ Allaqachon hal qilingan!", show_alert=True)
        return
    plan = pay["plan"]
    db.resolve_payment(pay_id, "approved")
    db.give_sub(pay["user_id"], plan)
    plan_info = db.PLANS.get(plan, {"label": "1 Oy"})
    await safe_edit_caption(ctx, q.message.chat_id, q.message.message_id,
...
async def cb_pay_no(update: Update, ctx):
    ...
    try:
        await ctx.bot.send_message(
            pay["user_id"],
            "❌ To'lovingiz tasdiqlanmadi. Qayta urinib ko'ring.",
            reply_markup=kb.user_kb()
        )
    except Exception:
        pass
...
async def msg_stats(update: Update, ctx):
    ...
    text = (
        ...
        "⏳ Kutilayotgan to'lovlar: <b>" + str(s["pending"]) + " ta</b>\n"
        "✅ Tasdiqlangan to'lovlar: <b>" + str(s["approved"]) + " ta</b>"
    )
    await update.message.reply_text(text, parse_mode=H)
...
async def cb_mv_add(update: Update, ctx):
    ...
    await q.answer()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "🎬 <b>Kino qo'shish</b>\n\nKino kodini kiriting:", parse_mode=H)
    await ctx.bot.send_message(
        q.from_user.id,
...
async def st_mv_code(update: Update, ctx):
    ...
    if existing:
        title = existing["title"] or "Noma'lum"
        await update.message.reply_text(
            "⚠️ <b>" + code + "</b> kodi mavjud!\nSarlavha: " + title + "\n\nYangi Message ID yuboring:",
            parse_mode=H, reply_markup=cancel_kb()
        )
    else:
        await update.message.reply_text(
            "✅ Kod: <b>" + code + "</b>\n\nEndi Message ID yuboring:\n"
            "Kanalda postga o'ng klik → Copy Link → oxiridagi raqam",
            parse_mode=H, reply_markup=cancel_kb()
        )
...
async def st_mv_msgid(update: Update, ctx):
    ...
    await update.message.reply_text(
        "📝 Kino sarlavhasini yuboring:\n(Sarlavhasiz qo'shish uchun  —  yuboring)",
        reply_markup=cancel_kb()
    )
    return S_MV_TITLE
...
async def st_mv_title(update: Update, ctx):
    ...
    await update.message.reply_text(
        ...
        parse_mode=H, reply_markup=ReplyKeyboardRemove()
    )
    await update.message.reply_text("Kinolar menyusi:", reply_markup=kb.movies_kb())
    ctx.user_data.clear()
    return END
...
async def cb_mv_pro(update: Update, ctx):
    ...
    await safe_edit(
        ...
        parse_mode=H, markup=kb.pro_manage_kb()
    )
...
async def cb_mv_set_pro(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    ...
                    parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, "Kino kodini kiriting:", reply_markup=cancel_kb())
    return S_PRO_SET
...
async def st_pro_set(update: Update, ctx):
    ...
    movie = db.get_movie(code)
    if not movie:
        await update.message.reply_text("❌ " + code + " topilmadi! Qayta kiriting:", reply_markup=cancel_kb())
        return S_PRO_SET
...
async def st_ed_code(update: Update, ctx):
    ...
    await update.message.reply_text("Yangi Message ID yuboring:", reply_markup=cancel_kb())
    return S_ED_MSGID
...
async def st_ed_msgid(update: Update, ctx):
    ...
    await update.message.reply_text("Yangi sarlavha yuboring (o'zgarmasa  —  yuboring):", reply_markup=cancel_kb())
    return S_ED_TITLE
...
async def st_ed_title(update: Update, ctx):
    ...
    await update.message.reply_text("✅ Kino yangilandi!", reply_markup=ReplyKeyboardRemove())
    await update.message.reply_text("Kinolar menyusi:", reply_markup=kb.movies_kb())
    ctx.user_data.clear()
    return END
...
async def st_del(update: Update, ctx):
    ...
    if db.del_movie(code):
        await update.message.reply_text(
            "✅ <b>" + code + "</b> o'chirildi!", parse_mode=H,
            reply_markup=ReplyKeyboardRemove()
        )
        await update.message.reply_text("Kinolar menyusi:", reply_markup=kb.movies_kb())
        return END
...
async def cb_mv_list(update: Update, ctx):
    ...
    if not movies:
        await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                        "📋 Kinolar bazasi bo'sh.", markup=kb.movies_kb())
        return
    text = "📋 <b>Kinolar ro'yxati</b>\n\n"
    for m in movies:
        pro = " 💎" if m["is_pro"] else ""
        title = m["title"] if m["title"] else "—"
        text += "• <code>" + m["code"] + "</code>" + pro + " — " + title + " (" + str(m["views"]) + " ko'rish)\n"
    if len(text) > 4000:
        text = text[:4000] + "\n\n... va boshqalar"
    await safe_edit(
        ctx, q.message.chat_id, q.message.message_id, text, parse_mode=H,
        markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Orqaga", callback_data="mv_back")]])
    )
...
async def cb_mv_back(update: Update, ctx):
    ...
    await q.answer()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "🎬 <b>Kinolar boshqaruvi</b>", parse_mode=H, markup=kb.movies_kb())
...
async def cb_ch_add(update: Update, ctx):
    ...
    await safe_edit(
        ...
        parse_mode=H, markup=kb.channel_type_kb()
    )
...
async def cb_cht_telegram(update: Update, ctx):
    ...
    await safe_edit(
        ...
        parse_mode=H,
        markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Orqaga", callback_data="ch_add")]])
    )
    await ctx.bot.send_message(
        q.from_user.id,
        "📢 Kanal ID yoki username kiriting:",
...
async def cb_cht_private(update: Update, ctx):
    ...
    await ctx.bot.send_message(q.from_user.id, "Kanal ID kiriting:", reply_markup=cancel_kb())
    return S_CH_ID
...
async def cb_cht_link(update: Update, ctx):
    ...
    await ctx.bot.send_message(q.from_user.id, "Havola nomini kiriting:", reply_markup=cancel_kb())
...
async def st_ch_id(update: Update, ctx):
    ...
    if ch_type in ("telegram", "private"):
        if not (text.startswith("@") or text.startswith("-100")):
            await update.message.reply_text(
                "❌ Noto'g'ri format!\nFaqat: @mykanal yoki -1001234567890",
                reply_markup=cancel_kb()
            )
            return S_CH_ID
    ctx.user_data["ch_id"] = text
    await update.message.reply_text("Kanal nomini kiriting (tugmada ko'rinadi):", reply_markup=cancel_kb())
    return S_CH_TITLE
...
async def st_ch_title(update: Update, ctx):
    ...
    if auto_link:
        ...
        await update.message.reply_text("Kanal havolasini kiriting:", reply_markup=cancel_kb())
    return S_CH_LINK
...
async def st_ch_link(update: Update, ctx):
    ...
    await update.message.reply_text("Kanallar menyusi:", reply_markup=kb.channels_kb())
    ctx.user_data.clear()
    return END
...
async def cb_ch_list(update: Update, ctx):
    ...
    if not chs:
        await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                        "📋 Hozircha kanallar yo'q.", markup=kb.channels_kb())
        return
    ...
    for i, c in enumerate(chs, 1):
        ...
        text += "\n"
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    text, parse_mode=H, markup=kb.channel_list_kb(chs))
...
async def cb_ch_del(update: Update, ctx):
    ...
    if not chs:
        await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                        "🗑️ O'chirish uchun kanallar yo'q.", markup=kb.channels_kb())
        return
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "🗑️ <b>Kanal o'chirish</b>\n\nO'chirish uchun kanal nomini bosing:",
                    parse_mode=H, markup=kb.channel_del_list_kb(chs))
...
async def cb_ch_back(update: Update, ctx):
    ...
    await safe_edit(
        ctx, q.message.chat_id, q.message.message_id,
        "🔒 <b>Majburiy obuna kanallari</b>\n\nHozirda: <b>" + str(len(chs)) + " ta</b> kanal",
        parse_mode=H, markup=kb.channels_kb()
    )
...
async def cb_adm_add(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    ...
                    "ID olish: @userinfobot", parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, "Admin ID sini kiriting:", reply_markup=cancel_kb())
    return S_ADM_ADD
...
async def st_adm_add(update: Update, ctx):
    ...
    uid = int(t)
    if uid in ADMIN_IDS:
        await update.message.reply_text("⚠️ Bu asosiy admin!", reply_markup=ReplyKeyboardRemove())
        return END
    db.add_admin(uid, "Admin " + str(uid))
    await update.message.reply_text(
        "✅ <code>" + str(uid) + "</code> admin qo'shildi!",
        parse_mode=H, reply_markup=ReplyKeyboardRemove()
    )
    await update.message.reply_text("Adminlar menyusi:", reply_markup=kb.admins_kb())
    try:
...
async def cb_adm_del(update: Update, ctx):
    ...
    await q.answer()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "🗑️ <b>Admin o'chirish</b>\n\nAdmin Telegram ID sini yuboring:", parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, "Admin ID sini kiriting:", reply_markup=cancel_kb())
    return S_ADM_DEL
...
async def st_adm_del(update: Update, ctx):
    ...
    if db.del_admin(uid):
        ...
        await update.message.reply_text("❌ Bu ID topilmadi.", reply_markup=cancel_kb())
    return END
...
async def cb_adm_list(update: Update, ctx):
    ...
    await safe_edit(
        ...
        "Jami: <b>" + str(total) + " ta</b>\n\n"
        "👑 Asosiy: " + main_admins,
        parse_mode=H, markup=kb.admins_kb()
    )
...
async def msg_broadcast(update: Update, ctx):
    ...
    await update.message.reply_text(
        "📨 <b>Xabar yuborish</b>\n\nJami: <b>" + str(count) + " ta</b> foydalanuvchi\n\nXabar turini tanlang:",
...
async def cb_bc_text(update: Update, ctx):
    ...
    await q.answer()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "📝 <b>Matnli xabar yuborish</b>\n\nXabar matnini yuboring:", parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, "Xabar matnini yuboring:", reply_markup=cancel_kb())
    return S_BC_TEXT
...
async def cb_bc_fwd(update: Update, ctx):
    ...
    await q.answer()
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "🔄 <b>Forward xabar yuborish</b>\n\nForward qilinadigan xabarni yuboring:", parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, "Forward qilinadigan xabarni yuboring:", reply_markup=cancel_kb())
    return S_BC_FWD
...
async def st_bc_text(update: Update, ctx):
    ...
    for i, uid in enumerate(users):
        ...
        if (i + 1) % 30 == 0:
            try:
                await safe_edit(ctx, msg.chat_id, msg.message_id,
    ...
    await ctx.bot.send_message(update.effective_user.id, "Admin panel:", reply_markup=kb.admin_kb())
    return END
...
async def msg_settings(update: Update, ctx):
    ...
    text = (
        ...
        "📅 3 Oy: " + "{:,}".format(int(price_3m)) + " so'm\n"
        "📅 1 Yil: " + "{:,}".format(int(price_1y)) + " so'm\n\n"
        "🎬 <b>Kino kanali:</b> <code>" + movch + "</code>\n\n"
        "💳 <b>To'lov kartalari:</b>\n"
        "UzCard: <code>" + uzcard + "</code>\n"
        "Humo: <code>" + humo + "</code>\n"
        "Visa: <code>" + visa + "</code>\n"
        "Egasi: " + owner + "\n\n"
        "👥 <b>Referral:</b> har " + refbonus + " ta = 1 oy premium\n\n"
        "📝 <b>Xush kelibsiz:</b>\n" + welcome_short + "..."
    )
...
async def cb_st_cards(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    ...
                    parse_mode=H, markup=kb.cards_kb())
...
async def cb_sc_card(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    "✏️ <b>" + name + "</b>\n\nYangi qiymatni yuboring:", parse_mode=H)
    await ctx.bot.send_message(q.from_user.id, name + " kiriting:", reply_markup=cancel_kb())
    return S_ST_CARD
...
async def st_save_card(update: Update, ctx):
    ...
    await update.message.reply_text("Sozlamalar:", reply_markup=kb.settings_kb())
    ctx.user_data.clear()
    return END
...
async def cb_st_prices(update: Update, ctx):
    ...
    await safe_edit(ctx, q.message.chat_id, q.message.message_id,
                    text, parse_mode=H, markup=kb.prices_kb())
...
async def cb_sp(update: Update, ctx):
    ...
    await safe_edit(
        ctx, q.message.chat_id, q.message.message_id,
        "✏️ <b>" + label + " narxi</b>\n\nHozirgi: " + "{:,}".format(int(current)) + " so'm\n\nYangi narx yuboring:",
        parse_mode=H
    )
    await ctx.bot.send_message(q.from_user.id, label + " uchun yangi narx:", reply_markup=cancel_kb())
    return S_ST_PRICE
...
async def cb_st_movch(update: Update, ctx):
    ...
    await ctx.bot.send_message(q.from_user.id, "Kino kanal ID kiriting:", reply_markup=cancel_kb())
    return S_ST_MOVCH


async def st_save_movch(update: Update, ctx):
    db.ss("movie_ch", update.message.text.strip())
    await update.message.reply_text("✅ Kino kanal saqlandi!", reply_markup=ReplyKeyboardRemove())
    await update.message.reply_text("Sozlamalar:", reply_markup=kb.settings_kb())
    return END
...
async def cb_st_welcome(update: Update, ctx):
    ...
    await ctx.bot.send_message(q.from_user.id, "Yangi xush kelibsiz matnini yuboring:", reply_markup=cancel_kb())
    return S_ST_WELCOME
...
async def cb_st_refbonus(update: Update, ctx):
    ...
    await ctx.bot.send_message(q.from_user.id, "Yangi referral bonus sonini yuboring:", reply_markup=cancel_kb())
...
def main():
    ...
    # Cancel filter
    cancel_filter = filters.Regex("^" + CANCEL_BTN + "$")
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_MV_CODE:  [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_mv_code)],
            S_MV_MSGID: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_mv_msgid)],
            S_MV_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_mv_title)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_ED_OLD:   [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ed_old)],
            S_ED_CODE:  [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ed_code)],
            S_ED_MSGID: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ed_msgid)],
            S_ED_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ed_title)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={S_DEL: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_del)]},
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
        ], per_user=True,
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_PRO_SET:   [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_pro_set)],
            S_PRO_UNSET: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_pro_unset)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_CH_ID:    [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ch_id)],
            S_CH_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ch_title)],
            S_CH_LINK:  [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_ch_link)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_ADM_ADD: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_adm_add)],
            S_ADM_DEL: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_adm_del)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_BC_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_bc_text)],
            S_BC_FWD:  [MessageHandler(filters.ALL  & ~filters.COMMAND & ~cancel_filter, st_bc_fwd)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            S_ST_CARD:     [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_save_card)],
            S_ST_PRICE:    [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_save_price)],
            S_ST_MOVCH:    [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_save_movch)],
            S_ST_WELCOME:  [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_save_welcome)],
            S_ST_REFBONUS: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~cancel_filter, st_save_refbonus)],
        },
        fallbacks=[
            MessageHandler(cancel_filter, cancel),
            CommandHandler("cancel", cancel),
    ...
    if os.environ.get("RENDER") == "true":
        logging.info("Webhook (Render) ishga tushmoqda...")
        app.run_webhook(
        ...
        logging.info("Polling (local) ishga tushmoqda...")
